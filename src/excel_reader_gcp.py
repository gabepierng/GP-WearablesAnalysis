...
def identify_gait_events(position_not_ends, heel_contacts, toe_contacts, seq):
...
'''
def calc_spatio_temp_params(gait_events, pelvis_position, foot_position, frame_rate, knee_angles, hip_angles):
    ...
    avg_dist_walked = np.mean(calc_distances(pelvis_position[r_gait_events[0,:]],
    ...
    avg_walk_time = (np.mean(r_gait_events[2,:] - r_gait_events[0,:]) / frame_rate)
...
'''
def calc_kinematic_params(gait_events, pelvis_orientation, hip_angles, knee_angles, ankle_angles, sternum_orientation):
...
'''
def calc_signal_params(lower_body_strides):
...
'''
def divide_into_strides(gait_events, signal, side, signal_num):
    if(side == 'r'):
...
def time_align_mvn_and_dot(mvn_gyro_data, xsens_dot_data, gait_events, is_dot_data_time_synced=True,):
    ...
    for sensor_location in xsens_dot_data:
        ...
        test_filt_1 = scipy.signal.filtfilt(b20, a20, gyro_data[mvn_corresponding_sensor, :, 1])
        if(is_dot_data_time_synced):
        ...
        # min_mvn_signal = -1 * np.min(gyro_data[mvn_corresponding_sensor, :, 1])
        ...
        # mvn_peaks, _ = find_peaks(-gyro_data[mvn_corresponding_sensor, :, 1], height = 0.3 * min_mvn_signal, distance = 30)
