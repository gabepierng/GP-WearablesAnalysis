...
#     return gait_profile_score, gait_variability_score
...
def calc_gait_profile_score(part_kinematics, control_kinematics):
    ...
    def create_kin_profile(kinematics_dict):
        ...
        kinematics_arr['hip_flexion_L'] = np.mean(kinematics_dict['hip_angle'][0][:, :, 0], axis=0).transpose()  # left hip flexion/extension
        kinematics_arr['hip_abduction_L'] = np.mean(kinematics_dict['hip_angle'][0][:, :, 1], axis=0).transpose()  # left hip abduction/adduction
        kinematics_arr['hip_rotation_L'] = np.mean(kinematics_dict['hip_angle'][0][:, :, 2], axis=0).transpose()  # left hip internal/external rotation
        
        kinematics_arr['hip_flexion_R'] = np.mean(kinematics_dict['hip_angle'][1][:, :, 0], axis=0).transpose()  # right hip flexion/extension
        kinematics_arr['hip_abduction_R'] = np.mean(kinematics_dict['hip_angle'][1][:, :, 1], axis=0).transpose()  # right hip abduction/adduction
        kinematics_arr['hip_rotation_R'] = np.mean(kinematics_dict['hip_angle'][1][:, :, 2], axis=0).transpose()  # right hip internal/external rotation
        
        kinematics_arr['knee_flexion_L'] = np.mean(kinematics_dict['knee_angle'][0][:, :, 2], axis=0).transpose()  # left knee flexion/extension
        kinematics_arr['knee_flexion_R'] = np.mean(kinematics_dict['knee_angle'][1][:, :, 2], axis=0).transpose()  # right knee flexion/extension
        
        kinematics_arr['ankle_flexion_L'] = np.mean(kinematics_dict['ankle_angle'][0][:, :, 2], axis=0).transpose()  # left ankle dorsiflexion/plantarflexion
        kinematics_arr['ankle_flexion_R'] = np.mean(kinematics_dict['ankle_angle'][1][:, :, 2], axis=0).transpose()  # right ankle dorsiflexion/plantarflexion
        
        kinematics_arr['foot_progression_L'] = np.mean(kinematics_dict['ankle_angle'][0][:, :, 1], axis=0).transpose()  # left foot progression
        kinematics_arr['foot_progression_R'] = np.mean(kinematics_dict['ankle_angle'][1][:, :, 1], axis=0).transpose()  # right foot progression
